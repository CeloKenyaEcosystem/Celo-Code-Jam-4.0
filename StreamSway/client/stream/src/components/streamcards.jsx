    ...
    <>
      {freelancers?.map((employee, index) => (
        ...
        <input
          ...
          value={flowRate}
          onChange={handleFlowRateChange}
           className="border-b md:w-3/4 border-gray-600 outline-none bg-transparent text-center text-black "
        />
...
{openstart?<div className="flex justify-between w-full"> <button className="text-green-600 md:text-xl  text-xs" onClick={()=>{handleStartStream(employee.userAddress,weiPerSeconds)}}>Start</button>
          <button className="text-red-600 md:text-xl  text-xs" onClick={()=>{setCardIndex(null)}} >Cancel</button></div>:""}
          {openUpdate?<div className="flex justify-between w-full"><button className="text-green-600 md:text-xl  text-xs" onClick={()=>{handleUpdateStream(employee.userAddress)}}>update</button>
          <button className="text-red-600 md:text-xl  text-xs" onClick={()=>{setUpdateCardIndex(null)}} >Cancel</button></div>:""}
